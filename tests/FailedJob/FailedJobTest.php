...
it('return failed job list', function () {
    insertFailedJob();
    $response = get('/api/motionpanel/failed-jobs');
...
it("success delete a failed job", function () {
    insertFailedJob();
    $response = delete('/api/motionpanel/failed-jobs/5ae7c1ad-5e9d-44f8-b7bb-04b7c42cad40');
    $response->assertStatus(200);
    $response_jobs = get('/api/motionpanel/failed-jobs');
    expect($response_jobs['data'])->toHaveCount(0);
});

it("success retry a failed job", function () {
    insertFailedJob();
    $response = post('/api/motionpanel/failed-jobs/5ae7c1ad-5e9d-44f8-b7bb-04b7c42cad40/retry');
    $response->assertStatus(200);
    $response_jobs = get('/api/motionpanel/failed-jobs');
    expect($response_jobs['data'])->toHaveCount(0);
});